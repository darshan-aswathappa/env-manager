...
describe('Sidebar', () => {
  it('renders project names', () => {
    ...
    render(<Sidebar projectTree={tree} selectedId={null} onSelect={vi.fn()} onDelete={vi.fn()} onAdd={vi.fn()} onAddSubProject={vi.fn()} />)
  ...
  it('calls onSelect with project id when clicked', () => {
    ...
    render(<Sidebar projectTree={tree} selectedId={null} onSelect={onSelect} onDelete={vi.fn()} onAdd={vi.fn()} onAddSubProject={vi.fn()} />)
    screen.getByText('my-api').closest('[role="button"]')!.dispatchEvent(new MouseEvent('click', { bubbles: true }))
    expect(onSelect).toHaveBeenCalledWith('p1')
  })

  it('shows empty state when no projects', () => {
    render(<Sidebar projectTree={[]} selectedId={null} onSelect={vi.fn()} onDelete={vi.fn()} onAdd={vi.fn()} onAddSubProject={vi.fn()} />)
    expect(screen.getByText(/No projects yet/i)).toBeInTheDocument()
  })

  it('marks selected project as active', () => {
    const tree = [makeNode('p1', 'selected-proj')]
    render(<Sidebar projectTree={tree} selectedId="p1" onSelect={vi.fn()} onDelete={vi.fn()} onAdd={vi.fn()} onAddSubProject={vi.fn()} />)
  ...
  it('calls onDelete when remove button is clicked', () => {
    ...
    render(<Sidebar projectTree={tree} selectedId={null} onSelect={vi.fn()} onDelete={onDelete} onAdd={vi.fn()} onAddSubProject={vi.fn()} />)
  ...
  it('calls onAddSubProject when sub-project button is clicked', () => {
    ...
    const tree = [makeNode('p1', 'my-project')]
    render(<Sidebar projectTree={tree} selectedId={null} onSelect={vi.fn()} onDelete={vi.fn()} onAdd={vi.fn()} onAddSubProject={onAddSubProject} />)
  ...
  it('calls onAdd when add project button is clicked', () => {
    const onAdd = vi.fn()
    render(<Sidebar projectTree={[]} selectedId={null} onSelect={vi.fn()} onDelete={vi.fn()} onAdd={onAdd} onAddSubProject={vi.fn()} />)
  ...
  it('shows var count correctly for singular', () => {
    ...
    render(<Sidebar projectTree={[nodeWithVar]} selectedId={null} onSelect={vi.fn()} onDelete={vi.fn()} onAdd={vi.fn()} onAddSubProject={vi.fn()} />)
