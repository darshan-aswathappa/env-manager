...
fn is_main_thread() -> bool {
    #[cfg(target_vendor = "apple")]
    {
        ...
        // macOS 26 (Tahoe): during `applicationDidFinishLaunching:` via AE handling,
        // pthread_main_np() returns -1 ("init not yet complete") even on the main thread.
        // Apple's header says "non-zero if main thread", so use != 0.
